...
4. **数字人生成** (`ad_orchestrator.py`)
   - **三步ComfyUI工作流**：
     1. 图片清洗（Qwen Image Edit - 去除文字/水印）
     2. 声音克隆（IndexTTS2 - 克隆说话者声音）
     3. 数字人视频（InfiniteTalk - 生成说话的数字人）
...
### 关键依赖
- **ComfyUI服务器必须运行**在配置的host:port上，用于阶段4-5
...
3. `InfiniteTalk数字人-图生视频-API-1114.json` - InfiniteTalk（数字人生成）
